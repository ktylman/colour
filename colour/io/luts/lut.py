...
class AbstractLUT(ABC):
    ...
    def __init__(self,
        ...
        self._dimensions = 0 if dimensions is None else dimensions

        self._table: NDArray = self.linear_table(
            0 if size is None else size,
            np.array([]) if domain is None else domain)
        self.table = (self._table
                      if table is None else table)  # type: ignore[assignment]
        # TODO: Remove pragma when https://github.com/python/mypy/issues/3004
        # is resolved.
        self._domain: NDArray = np.array([])
        self.domain = (self._domain if domain is None else
                       domain)  # type: ignore[assignment]
        self._comments: List = []
        self.comments = (self._comments if comments is None else
                         comments)  # type: ignore[assignment]
...
class LUT1D(AbstractLUT):
    ...
    def linear_table(size: Optional[IntegerOrArrayLike] = None,
        ...
        domain = np.array([0, 1]) if domain is None else as_float_array(domain)
...
class LUT3x1D(AbstractLUT):
    ...
    def linear_table(size: Optional[IntegerOrArrayLike] = None,
        ...
        domain = (np.array([[0, 0, 0], [1, 1, 1]])
                  if domain is None else as_float_array(domain))
...
class LUT3D(AbstractLUT):
    ...
    def apply(self, RGB: ArrayLike, **kwargs: Any) -> NDArray:
        ...
        return RGB_i  # type: ignore[return-value]
